...
function [nd_array, nd_bins,unique_bins] = histnd2(nd_data, nd_ranges, bin_size)
...
if ~isempty(find(isnan(nd_data)))
    nanFlag=1;
    allData=NaN(size(nd_data));
    nonNanInd=find(~isnan(nd_data));
    nd_data=nd_data(nonNanInd);
else nanFlag=0;
end
...
unique_bins=zeros(length(nd_array),1);
unique_bins(:)=NaN;
binsInd=unique(nd_bins);
unique_bins(binsInd)=binsInd;

if nanFlag==1;
     allData(nonNanInd)=nd_bins;
     nd_array=allData;
end
